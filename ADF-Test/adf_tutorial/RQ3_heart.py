...
from adf_utils.config import heart
...
#             config.gpu_options.per_process_gpu_memory_fraction = 0.8
        ...
        for key in all_probs.keys():
            probs = np.array(all_probs[key]).reshape((14,2))[:,1:].reshape((14))
...
if __name__ == '__main__':
    flags.DEFINE_string("dataset", "heart", "the name of dataset")
    flags.DEFINE_string('model_path', '../models/', 'the path for testing model')
    flags.DEFINE_list('sens_params',[2,1],'sensitive parameters index.1 for age, 9 for gender, 8 for race')
    tf.app.run()
