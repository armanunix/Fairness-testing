...
def is_discriminatory(sess,preds,z,x, similar_x, model):
    labels=model_argmax(sess, z, preds, np.array(similar_x))
...
def max_diff(sess,preds,z,x, similar_x, model):
    # select a similar instance such that the DNN outputs on them are maximally different
    y_pred_proba=model_prediction(sess, z, preds, np.array([x]))
    #y_pred_proba = model(tf.constant([x]))
    def distance(x_new):
        return np.sum(np.square(y_pred_proba - model_prediction(sess, z, preds, np.array([x_new]))))
    max_dist = 0.0
